...
// mock 数据占位，防止找不到 @/mock/evaluation
const applicationTemplates = [
  { id: 1, name: '模板A', type: '类型1' },
  { id: 2, name: '模板B', type: '类型2' }
];
const applicationRequirements = [
  { name: '必填项1', required: true, description: '描述1' },
  { name: '选填项2', required: false, description: '描述2' }
];
const herbEvaluations = [
  { id: 1, herbName: '人参', latinName: 'Panax ginseng', origin: '吉林', evaluationCount: 10, averageScore: 4.8, qualityLevel: '优秀', lastEvaluated: '2025-06-01', evaluationProgress: 100, evaluationStatus: '已完成' },
  { id: 2, herbName: '黄芪', latinName: 'Astragalus membranaceus', origin: '甘肃', evaluationCount: 8, averageScore: 4.2, qualityLevel: '良好', lastEvaluated: '2025-05-20', evaluationProgress: 80, evaluationStatus: '进行中' }
];
...
export default function ApplicationPage() {
  ...
  const [selectedTemplate, setSelectedTemplate] = useState<{ id: number; name: string; type: string } | null>(null);
  ...
  const handleTemplateSelect = (templateId: number) => {
    const template = applicationTemplates.find((t) => t.id === templateId) || null;
    setSelectedTemplate(template);
    message.success(`已选择申报模板：${template?.name}`);
  };

  // 处理药材选择
  const handleHerbSelect = (selectedRowKeys: number[]) => {
  ...
  const herbColumns = [
    {
      ...
      render: (text: string, record: any) => (
        <Space>
          ...
          <Typography.Text type="secondary">({record.latinName})</Typography.Text>
        </Space>
      ),
    },
    {
    ...
    },
    {
      ...
      render: (text: string) => {
        ...
        switch (text) {
          ...
          case '良好': color = 'blue'; break;
          case '合格': color = 'orange'; break;
          case '待改进': color = 'volcano'; break;
          case '不合格': color = 'red'; break;
      ...
      content: (
        <Card>
          ...
          <Table 
            rowSelection={{
              type: 'checkbox',
              onChange: (selectedRowKeys: React.Key[]) => handleHerbSelect(selectedRowKeys as number[]),
            }}
            columns={herbColumns}
