...
function AdminHerbCategoryDetailPage() {
  ...
  const { data: herbData, loading: herbLoading } = useAxios<HerbResponse>(
    `/herb-info-service/herbs/info/${name}`,
    "GET",
    null,
    {}
  );

  const updateHerbInfo = (values: { origin: string; des: string }) => {
    axiosInstance
      .put(`/herb-info-service/herbs/info/${id}`, {
        name: name,
  ...
  useEffect(() => {
    //@ts-ignore
    let herbDataTyped = (herbData as HerbResponse)
    if (herbData && herbDataTyped.code === 0) {
        //@ts-ignore
  ...
  useEffect(() => {
    axiosInstance
      .get(`/herb-info-service/herbs/info/${name}`)
      .then((res) => {
    ...
    <div>
      <AdminBreadcrumb
        items={[
          ...
          {
            title: "详细信息",
            href: `/admin/herb/category/detail?name=${name}`,
          },
        ]}
      />
        ...
        <div className="my-2">
          ...
          {imageUrl && (
            <img
              ...
              width="50rem"
              alt="preview"
              className="aspect-square object-cover overflow-auto"
            />
          )}
        </div>
