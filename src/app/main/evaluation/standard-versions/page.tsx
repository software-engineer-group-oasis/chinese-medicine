...
import { 
  ...
  Input, Select, DatePicker, Divider, Typography, Breadcrumb,
  Timeline, Tabs, Badge, Popconfirm, message, Drawer, List, Avatar
} from 'antd';
import { 
  ArrowLeftOutlined, PlusOutlined, EditOutlined, 
  HistoryOutlined, CheckCircleOutlined, CloseCircleOutlined,
  ExclamationCircleOutlined, EyeOutlined, CompareOutlined,
...
export default function StandardVersionsPage() {
  // 状态管理
  ...
  // 模拟评价标准版本数据
  const [standardVersions, setStandardVersions] = useState([
    {
      id: '1',
      name: '通用中药材评价标准',
      version: 'v3.0',
  ...
  // 处理版本选择
  const handleVersionSelect = (record, selected) => {
    if (selected) {
      if (selectedVersions.length < 2) {
  ...
  // 处理复制版本
  const handleCopyVersion = (record) => {
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (status) => {
        ...
        return <Badge status={status === 'active' ? 'success' : status === 'archived' ? 'default' : 'warning'} text={text} />;
      },
    },
    {
      title: '创建者',
    ...
    },
    {
      ...
      render: (_, record) => (
        <Space size="small">
          <Button type="link" icon={<EditOutlined />} onClick={() => handleAddOrEdit(record)}>编辑</Button>
          <Button type="link" icon={<HistoryOutlined />} onClick={() => handleViewHistory(record)}>历史</Button>
          <Button type="link" icon={<CopyOutlined />} onClick={() => handleCopyVersion(record)}>复制</Button>
