...
export default function TrainingPage() {
  ...
  useEffect(() => {
    axiosInstance
      .get("/herb-training-service/material/all")
      .then((res) => {
        if (res.data.code === 0) {
  ...
  const hideWhenQuery = () => {
    return (
      <>
        ...
        {showLiveStreamBox && (
          <div
            className="fixed top-1/2 left-1/2 z-999 transform -translate-x-1/2 -translate-y-1/2
                bg-white px-4 py-6 rounded-md"
          >
            <div className="relative">
              <div
                className="absolute right-3 top-3 hover:text-red-600"
                onClick={() => setShowLiveStreamBox((prev) => !prev)}
              >
                <IoMdClose />
              </div>
              <div>直播间ID: </div>
              ...
              <Button type="link">
                <a
                  ...
                  href={`${process.env.NEXT_PUBLIC_HOST}/live-stream.html?roomId=${liveRoom.id}&token=${token}`}
                >
                  前往直播间
                </a>
              </Button>
    ...
    <>
      ...
      <div className="p-6">
        ...
        <div className="flex">
          ...
          {/* 右侧筛选结果列表 */}
          <div className="flex-1" id="result">
            <List
              ...
              renderItem={(item) => (
                <List.Item>
                  <Card
                    style={{
                      backgroundImage:
                        "linear-gradient(to right bottom , oklch(75% 0.183 55.934), oklch(85.2% 0.199 91.936) )",
                    }}
                    hoverable
                    className="h-full"
                  ...
                  >
                    <div className={"flex gap-2 my-2"}>
                      <div
                        className={
                          "flex gap-2  border-2  rounded-md px-2 py-1 items-center m-tag"
                        }
                      >
                        <GiHerbsBundle />
